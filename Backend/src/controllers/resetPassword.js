...
const generateToken = asyncHandlerFunction(async(req,res)=>
    {
        ...
        const sendEmailMessage = await sendMail(`reset your password using this email:${url} token:${token}`,email,"Password reset ");

        return res
        .status(200)
        .json( new ApiResponse(200,{token:token,url:url},"token created successfully"))
...
const resetPassword = asyncHandlerFunction(async(req,res)=>
    {
        ...
        await sendMail("Dear user, your password is reset successfully",mail,"Password Reset")
       

        return res
        .status(200)
        .json(new ApiResponse(200,{updatePassword},"password is updated successfully"))
    })
