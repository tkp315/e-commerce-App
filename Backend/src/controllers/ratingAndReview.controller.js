...
const ratingAndReview = asyncHandlerFunction(async (req, res) => {
  ...
  if (customerid !== uid)
    throw new ApiError(401, "you have not bought the product");
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { findProduct, findUser, ratingandreview }));
});
...
const getAllRatingAndReview = asyncHandlerFunction(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200,{ result },"we have succefully got rating and reviews"));
});

export { ratingAndReview, getAllRatingAndReview, getAverageRating };
