...
const addToCart = asyncHandlerFunction(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, { cartList: user.cartList }, "One product added successfully"));
});
...
const viewCart = asyncHandlerFunction(async (req, res) => {
    ...
    const user = await User.findById(uid).populate({
        path: 'cartList',
        select: 'title price description tags thumbnail'  // Select the fields you need
    });
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, { user,cartItems }, "Fetched products successfully"));
});
