   ...
   <HomeLayout>
    ...
    <div className="min-h-[90vh] flex flex-col gap-3 mt-2 mb-2 ">
        ...
        <div  className="w-full h-[55vh] flex flex-row items-center overflow-x-auto overflow-y-hidden">
         {snacks.map((e)=>
        {
            return (<div onClick={()=>navigate(`subcategory-products/${e.name.toLowerCase()}`)} key={index}>
                <SubCategoryCard 
                
                name={e.name} description={e.des} thumbnail={e.photo} price={e.price} size="60" cardHeight="48vh" imageHeight="28" ></SubCategoryCard>
            </div>)
        })}
        </div>
        </div>
         ...
         {cleaning.map((e)=>
        {
            return (<div key={index} className="">
                <SubCategoryCard name={e.name}  thumbnail={e.photo} size="60" cardHeight="30vh" imageHeight="28"  ></SubCategoryCard>
            </div>)
        })}
        </div>
        </div>
         ...
         {snacks.map((e)=>
        {
            return (<div key={index}>
                <SubCategoryCard name={e.name} description={e.des} thumbnail={e.photo} price={e.price} size="60" cardHeight="48vh" imageHeight="28"  ></SubCategoryCard>
            </div>)
        })}
        </div>
        </div>
