    ...
    try {
      const res = await toast.promise(
        axiosInstance.post("/product/get-all-products"),
        {
          ...
          error: (error) => {
            return (
              error.response.data.message || "Unable to fetch the products"
            );
          },
        }
      );
...
export const newProduct = createAsyncThunk(
  ...
  async (data, thunkAPI) => {
    try {
      const res = await toast.promise(
        axiosInstance.post("/product/add-product", data, {
        ...
        }),
        {
          ...
          error: (error) => {
            return (
              error.response?.data?.message || "Unable to list the products"
            );
          },
        }
      );
        ...
        axiosInstance.post("/product/addtocart", data),
        {
          ...
          error: (error) => {
            return (
              ...
              "Unable to add the product into cart"
            );
          },
        }
      );
...
export const viewCart = createAsyncThunk(
  ...
  async (thunkAPI) => {
    try {
      const res = await toast.promise(axiosInstance.post("/product/viewcart"), {
    ...
    builder
      ...
      .addCase(viewCart.fulfilled, (state, action) => {
        ...
        localStorage.setItem(
          "cartItems",
          action?.payload?.data?.cartItems?.length
        );
